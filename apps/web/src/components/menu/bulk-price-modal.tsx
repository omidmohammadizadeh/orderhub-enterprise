...
export function BulkPriceModal({ open, menuId, menuName, preview, totalItems, onClose }: Props) {
  ...
  const apply = useMutation({
    ...
    onError: (e: any) => toast.error(e?.message ?? 'Could not change the base prices'),
  });
  ...
  const rows = useMemo(
    () =>
      preview.slice(0, 5).map((p) => ({
        ...p,
        next:
          percent === null ? p.basePrice : Math.max(0, round2(p.basePrice * (1 + percent / 100))),
      })),
    [preview, percent],
  );
        ...
        <div className="flex items-start justify-between border-b border-zinc-200 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-zinc-500">
              Moves the <strong>base price</strong> of every product and size in this menu by one
              percentage. This is the price POS, your own site and every channel without an uplift
              all charge.
            </p>
          </div>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
          <div>
            ...
            <p className="mt-2 text-xs text-zinc-500">
              A minus reduces every price; a plus raises it. Prices are rounded to the nearest
              penny.
            </p>
          </div>
          ...
          <label className="flex cursor-pointer items-start gap-2.5 rounded-xl border border-zinc-200 p-4">
            ...
            <span className="text-sm text-zinc-700">
              ...
              <span className="mt-0.5 block text-xs text-zinc-500">
                Off by default — 5% of a 50p topping is 2p, and most extras are priced on a round
                number worth leaving alone. Free options stay free either way.
              </span>
            </span>
          </label>
                ...
                {rows.map((r) => (
                  ...
                  >
                    ...
                    <span className="flex shrink-0 items-center gap-2 tabular-nums">
                      <span className="text-zinc-400 line-through">{money(r.basePrice)}</span>
                      <ArrowRight className="h-3.5 w-3.5 text-zinc-300" />
                      <span className="font-semibold text-zinc-900">{money(r.next)}</span>
                    </span>
                  </div>
                ))}
              </div>
          ...
          {percent !== null && (
            <div className="flex gap-2.5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs leading-relaxed text-amber-900">
              ...
              <div>
                <strong>This cannot be undone.</strong> The old prices are overwritten, and because
                everything rounds to the penny, applying the opposite percentage afterwards will not
                restore them exactly. Any channel uplift you have set moves by the same percentage,
                so a +20% Uber price stays +20% of the new base.
              </div>
            </div>
          )}
        </div>
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className={`gap-1.5 text-white ${
                confirming ? 'bg-red-600 hover:bg-red-700' : 'bg-zinc-900 hover:bg-zinc-800'
              }`}
            >
