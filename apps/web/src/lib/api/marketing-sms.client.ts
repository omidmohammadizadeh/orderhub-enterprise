...
export const marketingSmsClient = {
  channels: () =>
    apiClient.get<ChannelCount[]>("/v1/marketing-sms/channels").then((r) => r.data),

  contacts: (params: { consent?: string; source?: string; search?: string; limit?: number } = {}) => {
  ...
  importFromCustomers: (sources: string[], consentedOnly: boolean) =>
    apiClient
      .post<ImportReport>("/v1/marketing-sms/contacts/import-from-customers", {
        sources, consentedOnly,
      })
      .then((r) => r.data),

  importRows: (rows: ImportRow[], source: string, assertConsent: boolean) =>
    apiClient
      .post<ImportReport>("/v1/marketing-sms/contacts/import-rows", { rows, source, assertConsent })
      .then((r) => r.data),

  addContact: (body: { phone: string; firstName?: string; lastName?: string }) =>
  ...
  campaigns: () =>
    apiClient.get<SmsCampaign[]>("/v1/marketing-sms/campaigns").then((r) => r.data),

  campaign: (id: string) =>
    apiClient.get<SmsCampaign>(`/v1/marketing-sms/campaigns/${id}`).then((r) => r.data),

  saveCampaign: (body: {
    id?: string; name: string; senderHeader?: string; body: string; audience?: any;
  }) => apiClient.post<SmsCampaign>("/v1/marketing-sms/campaigns", body).then((r) => r.data),

  preview: (body: { senderHeader?: string; body: string; audience?: any }) =>
