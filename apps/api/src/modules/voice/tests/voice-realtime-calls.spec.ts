...
describe('words the model is not allowed to rephrase', () => {
    ...
    const sim = new VoiceRealtimeSim({
      tools: {
        read_back_order: {
          result: 'Read it back.',
          sayNow:
            "So that's 1 PEPPERONI and 1 CHIPS, for collection. That comes to £10.70. Is that all correct?",
        } as any,
      },
    });
    await sim.answer();
  ...
  it('still insists when the words had to wait for a reply to finish', async () => {
    const sim = new VoiceRealtimeSim({
      tools: {
        place_order: {
          result: 'Placed.',
          sayNow: "That's all booked in, order number 4, 0, 1, 2.",
        } as any,
      },
    });
    await sim.answer();
  ...
  it('leaves ordinary answers to the model', async () => {
    ...
    const ask = sim.toModel.find((m) => m.type === 'response.create');
    // An ordinary follow-up says only what kind of reply it is: no script
    // to repeat, and its tools left alone.
    expect(ask.response).toEqual({ metadata: { origin: 'tool' } });
  });
});
...
describe('a keypress that answers the question just asked', () => {
  ...
  it('puts zero through to a person wherever it is pressed', async () => {
    const sim = new VoiceRealtimeSim({
      tools: {
        transfer_to_staff: { result: 'Putting you through.', turn: { transferTo: '+44191' } },
      },
    });
...
describe('how the line decides the caller has finished talking', () => {
  ...
  it('falls back to the other one if the account will not take it', async () => {
    ...
    sim.brain.deliver({
      ...
      error: {
        type: 'invalid_request_error',
        message: 'Unknown parameter: session.audio.input.turn_detection.eagerness',
      },
    });
    await new Promise((r) => setTimeout(r, 10));
