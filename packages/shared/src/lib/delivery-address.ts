...
//
// Lives in @orderhub/shared rather than in the API because the till reads an
// order back too: the POS loads an existing order to amend it, and its own
// blob-only reader hit this same trap from the other side — a marketplace
// order opened at the till showed no address, and the operator could neither
// see it nor save over it.
...
export interface DeliveryAddressParts {
  ...
  /**
   * The named delivery area, where a shop prices by area instead of by
   * postcode (the Gulf, mostly). Reported SEPARATELY as well as being folded
   * into line2, because the two readers want different things: a driver wants
   * it on the address line, and the till needs it on its own to look up the
   * zone. Null everywhere a shop prices by postcode.
   */
  area: string | null;
}
